...
const TicketUpload = React.memo(
  ({
    ...
    <div
      ...
      onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); onDragStart(); }}
      onDragLeave={(e) => { e.preventDefault(); e.stopPropagation(); onDragEnd(); }}
      onDrop={(e) => {
        e.preventDefault(); e.stopPropagation(); onDragEnd();
        ...
        <div>
          {ticketType === "application/pdf" ? (
            <div className="etm-pdf-preview">
              ...
              {ticketFile && <div className="etm-file-info">{ticketFile.name}</div>}
            </div>
          ) : (
...
function useTicketState(existingURL = "") {
  ...
  const initialType = existingURL ? (isPdf ? "application/pdf" : "image/url") : "";
  ...
  const processFile = useCallback((f) => {
    ...
    if (!ALLOWED_TYPES.includes(f.type)) {
      ...
      setFile(null); setPreview(null); setType("");
      return;
    }
    setFile(f);
    setType(f.type);
    setRemoveExisting(false);
    setFileError(null);
    if (f.type === "application/pdf") { setPreview("pdf"); return; }
  ...
  const remove = useCallback(() => {
    setFile(null); setPreview(null); setType("");
    setRemoveExisting(true); setFileError(null);
  }, []);

  return {
    file, preview, type, isDragging, setIsDragging,
    removeExisting, fileError, setFileError,
    processFile, remove,
  };
}
 ...
 */
const EditTravelModal = React.memo(({ registration, onSave, onCancel, isSaving }) => {
  /* ── form state ── */
  ...
  const departureTicket = useTicketState(registration?.departureTicketURL ?? "");
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      arrivalTrainName:       hasOtherArrival   ? "__other__" : (registration.arrivalTrainName   ?? ""),
      ...
      departureTrainName:     hasOtherDeparture ? "__other__" : (registration.departureTrainName ?? ""),
  ...
  const setArrivalMode = (val) =>
    setForm((p) => ({ ...p, arrivalTravelMode: val, arrivalTrainName: "", arrivalTrainNameOther: "" }));

  const setDepartureMode = (val) =>
    setForm((p) => ({ ...p, departureTravelMode: val, departureTrainName: "", departureTrainNameOther: "" }));
  ...
  const validate = () => {
    ...
    if (form.arrivalTravelMode === "Train" && form.arrivalTrainName === "__other__" && !form.arrivalTrainNameOther.trim())
    ...
    if (form.departureTravelMode === "Train" && form.departureTrainName === "__other__" && !form.departureTrainNameOther.trim())
    ...
    onSave({
      ...
      arrivalTrainName:
        form.arrivalTravelMode === "Train"
          ? form.arrivalTrainName === "__other__" ? "Others / अन्य" : form.arrivalTrainName
          : "",
      arrivalTrainNameOther:
        form.arrivalTravelMode === "Train" && form.arrivalTrainName === "__other__"
      ...
      departureTrainName:
        form.departureTravelMode === "Train"
          ? form.departureTrainName === "__other__" ? "Others / अन्य" : form.departureTrainName
          : "",
      departureTrainNameOther:
        form.departureTravelMode === "Train" && form.departureTrainName === "__other__"
          ? form.departureTrainNameOther.trim()
          : "",
      departureDate: form.departureDate,
      // ticket payloads
      arrivalTicketFile:      arrivalNeedsTicket  ? arrivalTicket.file  : null,
      removeArrivalTicket:    arrivalNeedsTicket  ? arrivalTicket.removeExisting : true,
      departureTicketFile:    departureNeedsTicket ? departureTicket.file : null,
      removeDepartureTicket:  departureNeedsTicket ? departureTicket.removeExisting : true,
    });
  };
  ...
  /* ── layout helpers ── */
  const modeIcons = { Train: "bi-train-front", Flight: "bi-airplane", Car: "bi-car-front" };
  ...
  const sectionHead = {
    ...
    textTransform: "uppercase", color: "#6c757d", marginBottom: "0.75rem",
  };
  ...
  return (
    <div style={backdropStyle} role="dialog" aria-modal="true">
      ...
      <div style={panelStyle}>
        ...
        {/* ── Header ── */}
        <div className="d-flex align-items-center px-4 py-3 border-bottom" style={{ flexShrink: 0 }}>
          <div
          ...
          <div className="flex-grow-1 min-width-0">
            ...
            <small className="text-muted text-truncate d-block">{registration.name}</small>
          </div>
          <button className="btn-close ms-3" onClick={onCancel} disabled={isSaving} aria-label="Close" />
        </div>
        ...
        <div className="px-4 py-3 overflow-auto flex-grow-1">
          ...
          {/* ════════ ARRIVAL ════════ */}
          <div className="rounded-3 p-3 mb-4" style={{ background: "#f0f7ff", border: "1px solid #c8deff" }}>
            <p style={sectionHead}>
            ...
            <div className="mb-3">
              ...
              <div className="d-flex gap-2 flex-wrap">
                {TRAVEL_MODES.map((m) => (
                  ...
                  >
                    <i className={`bi ${modeIcons[m]}`}></i>{m}
                  </button>
                ))}
              </div>
              {errors.arrivalTravelMode && <div className="text-danger small mt-1">{errors.arrivalTravelMode}</div>}
            </div>
            ...
            {isArrivalTrain && (
              <div className="mb-3">
                ...
                >
                  ...
                  {TRAIN_LIST.map((t) => <option key={t} value={t}>{t}</option>)}
                ...
                {form.arrivalTrainName === "__other__" && (
                  <input
                    ...
                    onChange={(e) => set("arrivalTrainNameOther", e.target.value)}
                  />
                )}
            ...
            <div className="mb-3">
              ...
              <div className="d-flex gap-2 flex-wrap">
                {ARRIVAL_DATES.map((d) => (
                  ...
                  >
                    <i className="bi bi-calendar2-event"></i>{d.label}
                  </button>
                ))}
              </div>
              {errors.arrivalDate && <div className="text-danger small mt-1">{errors.arrivalDate}</div>}
            </div>
            ...
            {arrivalNeedsTicket && (
              <div className="mb-1">
                <label className="form-label small fw-medium mb-1">
                  ...
                  <span className="text-muted fw-normal ms-1">(Image / PDF)</span>
                </label>
                <TicketUpload
                ...
                {(errors.arrivalTicket || arrivalTicket.fileError) && (
                  <div className="text-danger small mt-1">{errors.arrivalTicket || arrivalTicket.fileError}</div>
                )}
                {/* View existing ticket link */}
                {!arrivalTicket.file && !arrivalTicket.removeExisting && registration.arrivalTicketURL && (
                  <div className="mt-1">
                    <a
                    ...
                    {registration.arrivalTicketFileName && (
                      <span className="text-muted small ms-2">({registration.arrivalTicketFileName})</span>
                    )}
                  </div>
                )}
              </div>
          ...
          {/* ════════ DEPARTURE ════════ */}
          <div className="rounded-3 p-3" style={{ background: "#fff8f0", border: "1px solid #ffd8a0" }}>
            <p style={sectionHead}>
            ...
            <div className="mb-3">
              ...
              {errors.departureTravelMode && <div className="text-danger small mt-1">{errors.departureTravelMode}</div>}
            </div>
              ...
              <div className="mb-3">
                ...
                {form.departureTrainName === "__other__" && (
                  <input
                    ...
                    onChange={(e) => set("departureTrainNameOther", e.target.value)}
                  />
                )}
            ...
            <div className="mb-3">
              ...
              {errors.departureDate && <div className="text-danger small mt-1">{errors.departureDate}</div>}
            </div>
            ...
            {departureNeedsTicket && (
              <div className="mb-1">
                ...
                {(errors.departureTicket || departureTicket.fileError) && (
                  <div className="text-danger small mt-1">{errors.departureTicket || departureTicket.fileError}</div>
                )}
                {/* View existing ticket link */}
                {!departureTicket.file && !departureTicket.removeExisting && registration.departureTicketURL && (
                  <div className="mt-1">
                    <a
                    ...
                    {registration.departureTicketFileName && (
                      <span className="text-muted small ms-2">({registration.departureTicketFileName})</span>
                    )}
                  </div>
                )}
              </div>
        ...
        {/* ── Footer ── */}
        <div className="px-4 py-3 border-top d-flex gap-2 justify-content-end" style={{ flexShrink: 0 }}>
